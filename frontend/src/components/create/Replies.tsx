...
export default function Replies() {
	...
	useEffect(() => {
		const fetchData = async function() {
			...
			console.log("answerspromises", answersPromises)
			...
			answersData.forEach(answerObj => {
				// Iterate over each key-value pair in the answer object
				Object.entries(answerObj).forEach(([question_id, answer]) => {
					if (answer) {
						// Find the related question in QnA based on question_id (converted to number if needed)
						let relatedQuestion = QnA.find(q => q.id === Number(question_id)); // Ensure question_id matches q.id as a number
						if (relatedQuestion) {
							// Ensure the answers array exists for the question
							if (!relatedQuestion.answers) {
								relatedQuestion.answers = []; // Initialize answers array if it doesn't exist
							}
							// Push the answer to the correct question
			//@ts-ignore
							relatedQuestion.answers.push(answer); // Push the answer to the corresponding question
						}
					}
				});
			});
			...
			console.log(QnA);
			if (!surveyQnA || surveyQnA.length===0) {
				setSurveyQnA(QnA);
			}
	...
	return (
		<div className="m-auto w-[92%] p-6 bg-white rounded-lg shadow-lg mt-4">
			...
			<p className="text-gray-600">{surveyList.description}</p>
									...
									{item.answers.map((response, userIndex) => (
										<li key={userIndex} className="text-gray-800">
											<div className="flex">
												...
												/>
												<strong>{response.user}</strong>: {response}
											</div>
										</li>
									))}
								</ul>
